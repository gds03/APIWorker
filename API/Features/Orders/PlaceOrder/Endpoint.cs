...
{
    public List<PlaceOrderRequestItem> Products { get; set; } = [];
}
...
public class PlaceOrderEndpoint : Controller
{
    ...
    {
        var handlerRequest = PlaceOrderHandlerRequest.Create(accountId, request.Products.Select(p => (p.Sku, p.Amount)));
